    ...
    if type == "invoked" and args[0] == "mqttbroker":
        if len(args) == 1 or len(args) == 2 and args[1] == "-?" or args[1] == "/?":
            ...
            if len(args) == 2 and args[1] == "enable-service":
                mqttbroker.enable_service()
                print
                "Success: MQTT broker service enabled."
                print
                ""
                CLI.exit()
                ...
                mqttbroker.disable_service()
                print
                "Success: MQTT broker service disabled."
                print
                ""
                CLI.exit()
                ...
                if mqttbroker.remove_user(args[2]):
                    ...
                    print
                    "Success: removed user " + args[2] + "."
                else:
                    GUI.update("remove_user_fail", json.dumps({"username": args[2]}))
                    print
                    "Error: could not remove user " + args[2] + "."
...
def print_cli_usage():
    ...
    "Usage:"
    print
    "mqttbroker enable-service"
    print
    "mqttbroker disable-service"
    print
    "mqttbroker add-user <username> [password]"
    print
    "mqttbroker remove-user <username>"
    print
    "mqttbroker display-last-event"
    print
    "mqttbroker display-all-events"
    print
    ""
