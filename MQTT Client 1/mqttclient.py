...
def publish(topic, payload, qos):
    if not topic:
        ...
        if int(qos) > 0:
            ...
            if on_publish_callback is not None:
                on_publish_callback("Success",
                                    "Published message '" + payload + "' to topic '" + topic + "' with QoS level " + qos + ".",
                                    {"topic": topic, "payload": payload, "qos": int(qos)})
...
def on_connection_change(type):
    print
    "Connection change type: " + str(type)
...
def process_mqtt_packet(in_packet, pdu_info):
    ...
    if in_packet["cmd"] == "ID":
        ...
        if in_packet["returnCode"] == 0:
            if on_gui_update_callback is not None:
                on_gui_update_callback("connected",
                                       {"broker_address": broker_address, "username": username, "password": password,
        ...
        if on_message_received_callback is not None:
            on_message_received_callback("Success",
                                         "Received message '" + in_packet["payload"] + "' from topic '" + in_packet[
                                             "topic"] + "'.", in_packet)
        ...
        if on_gui_update_callback is not None:
            on_gui_update_callback("publish_success", {"topic": in_packet["topic"], "payload": in_packet["payload"],
                                                       "qos": in_packet["qos"]})

        if on_publish_callback is not None:
            on_publish_callback("Success", "Published message '" + in_packet["payload"] + "' to topic '" + in_packet[
                "topic"] + "' with QoS level " + str(in_packet["qos"]) + ".", in_packet)
